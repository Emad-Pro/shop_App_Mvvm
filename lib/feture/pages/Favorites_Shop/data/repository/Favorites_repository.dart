...
abstract class FavoritesRepository {
  ...
  Future<Either<Faliure, AddDeleteFavoritesProductModel>>
      fetchModeifyFavoritesProduct({required int id});
}
...
class FavoritesRepositoryImplementation extends FavoritesRepository {
  ...
  Future<Either<Faliure, FavoritesModel>> fetchFavoritesProduct() async {
    try {
      final response =
          await DioHelper.getData(url: 'favorites', token: UserData.uId);
  ...
  @override
  Future<Either<Faliure, AddDeleteFavoritesProductModel>>
      fetchModeifyFavoritesProduct({required int id}) async {
    try {
      final response = await DioHelper.postData(
          url: 'favorites', token: UserData.uId, data: {'product_id': id});
